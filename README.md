...
* Embeddings are stored in a Qdrant Cloud vector database for fast similarity searches.

#### 3. Multi-Query Retrieval:
* For each user query, LangChain generates multiple related queries automatically.
* Using vector similarity search, it retrieves the top 20 most relevant documents from Qdrant.

#### 4. BM25 Keyword-Based Search (Reversed Hybrid RAG):
* The 20 retrieved documents are reranked using BM25, which prioritizes the documents with stronger keyword matches.
